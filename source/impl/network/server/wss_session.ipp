...
namespace	impl::network::server
{
	...
	wss_session< session_type, handler_type >::wss_session( 
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::start()
	{
		...
		ssl_stream_socket().async_handshake(	
			...
			boost::asio::bind_executor(	
				_strand,
				boost::bind(	
					&wss_session::handshake, 
					this->shared_from_this(),
					boost::asio::placeholders::error ) ) );
	}

	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::handshake( const boost::system::error_code& ec_ )
	{
		...
		boost::asio::async_read_until(	
			...
			boost::asio::bind_executor(	
				_strand,
				boost::bind(	
					&wss_session::websocket_handshake_read,
					this->shared_from_this(),
					boost::asio::placeholders::error,
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::on_close()
	{
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::websocket_handshake_read( 
		...
		size_t bytes_transferred_ )
	{
		...
		boost::asio::async_write(	
			...
			boost::asio::bind_executor(	
				_strand,
				boost::bind(	
					&wss_session::websocket_handshake_write,
					this->shared_from_this(),
					ob,
					boost::asio::placeholders::error,
					boost::asio::placeholders::bytes_transferred ) ) );
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::websocket_handshake_write( 
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::read_header( 
		const boost::system::error_code& ec_, 
		size_t bytes_transferred_ )
	{
		if ( boost::system::errc::success != ec_ )
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::read_header_126( 
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::read_header_127( 
		uint8_t fin_rsv_opcode_, 
		const boost::system::error_code& ec_, 
		size_t bytes_transferred_ )
	{
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::read( 
		size_t length_, 
		uint8_t fin_rsv_opcode_, 
		const boost::system::error_code& ec_, 
		size_t bytes_transferred_ )
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::pong( uint8_t fin_rsv_opcode_ )
	{
		auto	res( alloc_outbound() );
		if ( !res )
		{
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::route( const packet::inbound_ptr_type& in_ )
	{
		if ( nullptr == in_ )
		{
			_fatal_log_( 
		...
		dispatch( 
			boost::bind( 
				&wss_session::route_poll, 
				this->shared_from_this() ) );
	}

	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::route_poll()
	{
	...
	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::write( 
		const boost::system::error_code& ec_, 
		size_t bytes_transferred_ )
	{
		if ( boost::system::errc::success != ec_ )
	...
	template < typename session_type, typename handler_type >
	const std::string&	wss_session< session_type, handler_type >::on_get_remote_ip()		const
	{
		return	get_remote_ip();
	}

	template < typename session_type, typename handler_type >
	uint16_t	wss_session< session_type, handler_type >::on_get_remote_port()		const
	{
		return	get_remote_port();
	}

	template < typename session_type, typename handler_type >
	bool	wss_session< session_type, handler_type >::on_is_connected()		const
	{
		return	is_connected();
	}

	template < typename session_type, typename handler_type >
	bool	wss_session< session_type, handler_type >::on_is_keep_alive_flag()	const
	{
		return	is_keep_alive_flag();
	}

	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::on_init_keep_alive_flag()
	{
		init_keep_alive_flag();
	}

	template < typename session_type, typename handler_type >
	void	wss_session< session_type, handler_type >::on_set_keep_alive_flag()
	{
		set_keep_alive_flag();
	}
	...
	boost::optional< packet::outbound_ptr_type >
		wss_session< session_type, handler_type >::on_alloc_outbound()
	{
		return	alloc_outbound();
	}

	template < typename session_type, typename handler_type >
	bool	wss_session< session_type, handler_type >::on_send( const packet::outbound_ptr_type& out_ )
	{
		if ( nullptr == out_ )
		{
	...
	template < typename session_type, typename handler_type >
	void			wss_session< session_type, handler_type >::on_post( const void_func_type& f_ )
	{
		post( f_ );
	}

	template < typename session_type, typename handler_type >
	void			wss_session< session_type, handler_type >::on_dispatch( const void_func_type& f_ )
	{
		dispatch( f_ );
	}

	template < typename session_type, typename handler_type >
	e_protocol_type	wss_session< session_type, handler_type >::on_protocol_type()	const
	{
		return	e_protocol_type::_wss_;
	}
}