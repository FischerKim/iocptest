...
static int same(const uint8_t *src, size_t n)
{
	while(n > 0 && *(src + n) == *src)
		n--;
	return n == 0 ? 1 : 0;
...
static __inline ui32 hashat(const uint8_t *src)
{
	ui32 fetch, hash;
...
static __inline void memcpy_up(uint8_t *dst, const uint8_t *src, ui32 n)
{
...
static __inline void update_hash(qlz_state_decompress *state, const uint8_t *s)
{
#if QLZ_COMPRESSION_LEVEL == 1
	ui32 hash;
	hash = hashat(s);
...
#if QLZ_COMPRESSION_LEVEL <= 2
static void update_hash_upto(qlz_state_decompress *state, uint8_t **lh, const uint8_t *max)
{
	while(*lh < max)
	{
		(*lh)++;
...
static size_t qlz_compress_core(const uint8_t *source, uint8_t *destination, size_t size, qlz_state_compress *state)
{
	const uint8_t *last_byte = source + size - 1;
	const uint8_t *src = source;
	uint8_t *cword_ptr = destination;
	uint8_t *dst = destination + CWORD_LEN;
	ui32 cword_val = 1U << 31;
	const uint8_t *last_matchstart = last_byte - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END; 
	ui32 fetch = 0;
	ui32 lits = 0;
...
static size_t qlz_decompress_core(const uint8_t *source, uint8_t *destination, size_t size, qlz_state_decompress *state, const uint8_t *history)
{
	const uint8_t *src = source + qlz_size_header((const char *)source);
	uint8_t *dst = destination;
	const uint8_t *last_destination_byte = destination + size - 1;
	ui32 cword_val = 1;
	const uint8_t *last_matchstart = last_destination_byte - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END;
	uint8_t *last_hashed = destination - 1;
	const uint8_t *last_source_byte = source + qlz_size_compressed((const char *)source) - 1;
	...
	for(;;) 
	{
		...
		if ((cword_val & 1) == 1)
		{
			...
			cword_val = cword_val >> 1;
			hash = (fetch >> 4) & 0xfff;
			offset2 = (const uint8_t *)(size_t)state->hash[hash].offset;
...
size_t qlz_compress(const void *source, char *destination, size_t size, qlz_state_compress *state)
{
...
#endif
	{
		...
		r = base + qlz_compress_core((const uint8_t *)source, (uint8_t*)destination + base, size, state);
#if QLZ_STREAMING_BUFFER > 0
		reset_table_compress(state);
#endif
		if(r == base)
	...
	{
		uint8_t *src = state->stream_buffer + state->stream_counter;

		memcpy(src, source, size);
		r = base + qlz_compress_core(src, (uint8_t*)destination + base, size, state);
	...
	if(base == 3)
	{
		*destination = (uint8_t)(0 | compressed);
		*(destination + 1) = (uint8_t)r;
		*(destination + 2) = (uint8_t)size;
	}
	else
	{
		*destination = (uint8_t)(2 | compressed);
...
size_t qlz_decompress(const char *source, void *destination, qlz_state_decompress *state)
{
...
#endif
	{
		if((*source & 1) == 1)
		{
			...
			dsiz = qlz_decompress_core((const uint8_t *)source, (uint8_t *)destination, dsiz, state, (const uint8_t *)destination);
		}
		else
		{
	...
	{
		uint8_t *dst = state->stream_buffer + state->stream_counter;
		if((*source & 1) == 1)
		{
			dsiz = qlz_decompress_core((const uint8_t *)source, dst, dsiz, state, (const uint8_t *)state->stream_buffer);
		}
		else
		{
